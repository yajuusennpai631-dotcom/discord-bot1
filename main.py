...
async def is_admin_or_allowed(interaction: discord.Interaction) -> bool:
    """管理者または許可ユーザーであるか確認する（一般ユーザーを弾く）"""
    ...
    await interaction.response.send_message("❌ このコマンドを実行する権限がありません（管理者または許可ユーザー専用です）。", ephemeral=True)
    return False
...
async def on_ready():
    ...
    for guild_id_str, config in all_data.items():
        ...
        print(f"  > メッセージ転送: {'有効' if config.get('from_channel') else '未設定'}")
    ...
    print("⚠️ 429エラー対策のため、起動時の自動同期(sync)を停止しました。")
...
async def sync_command(ctx):
    if ctx.author.guild_permissions.administrator:
        ...
        try:
            await bot.tree.sync()
            await ctx.send("✅ スラッシュコマンドの強制同期が完了しました！Discordアプリを再起動して確認してください。")
...
# 🌐 【一般解放】一般プレイヤーが検索エンジンやWikipediaを使って簡単に検索できるコマンド
@bot.tree.command(name="search", description="【便利ツール】各種検索サイトやWikipediaの検索リンクを生成します")
@discord.app_commands.choices(engine=[
    ...
    discord.app_commands.Choice(name="X /旧Twitter (トレンド)", value="x"),
...
async def search(interaction: discord.Interaction, engine: discord.app_commands.Choice[str], query: str):
    # 一般ユーザーも使用可能（チェック関数なし）
    eng = engine.value

    if eng == "wiki":
        await interaction.response.defer(ephemeral=False) # プレイヤー全員に見えるよう、ephemeral=Falseに変更
        try:
        ...
        embed = discord.Embed(
            ...
            description=f"「**{query}**」の検索ボタンを作成しました。\n\n🔗 **[ここをクリックして検索結果を開く]({urls[eng]})**",
...
async def my_scan(interaction: discord.Interaction, target_user: discord.User = None):
    # ターゲットユーザーが指定されている場合は、悪用防止のため「管理者・許可ユーザー」のみにする
    if target_user:
        ...
        # ターゲット指定がない（純粋なサーバー情報の確認）なら一般プレイヤーでも実行可能
        if not interaction.guild:
            await interaction.response.send_message("サーバー内で実行してください。", ephemeral=True)
            return
        g = interaction.guild
        embed = discord.Embed(title=f"📊 {g.name} サーバー統計ステータス", color=discord.Color.teal())
...
@bot.tree.command(name="my_scan_channels", description="【管理】サーバーのチャンネル構造とカスタム権限をスキャン")
...
@bot.tree.command(name="server_status", description="【管理】現在のサーバー設定状況を確認します")
...
@bot.tree.command(name="server_role_panel", description="【管理】選択されたチャンネル閲覧用のロール取得パネルを送信します")
async def server_role_panel(interaction: discord.Interaction, title: str = "ロール付与", description: str = "お好きなものを選んでください"):
    if not interaction.guild or not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("権限がありません。", ephemeral=True)
        return
    ...
    if not roles:
        await interaction.followup.send("❌ このチャンネルの権限設定に個別登録されているロールが見つかりませんでした。", ephemeral=True)
        return
    ...
    await interaction.followup.send("✅ ロール付与パネルを設置しました！", ephemeral=True)
