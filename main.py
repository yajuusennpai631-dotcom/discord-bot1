...
class GuildLeaveConfirmView(discord.ui.View):
    ...
    @discord.ui.button(label="脱退する", style=discord.ButtonStyle.danger)
    ...
    if eng == "wiki":
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=5) as response:
                ...
                embed = discord.Embed(title=f"Wikipedia · {data.get('title', query)}", description=data.get('extract', '概要なし'), color=discord.Color.blue())
        ...
        embed = discord.Embed(
            ...
            description=f"[検索結果を開く]({urls[eng]})",
...
async def my_scan(interaction: discord.Interaction, target_user: discord.User = None):
    if target_user:
        embed = discord.Embed(title="ユーザー情報", color=discord.Color.teal())
...
async def my_memo(interaction: discord.Interaction, action: discord.app_commands.Choice[str], content: str = None):
    ...
    if act == "add":
        ...
        memos = user_data.get("memos", [])
        embed = discord.Embed(title="個人メモ", color=discord.Color.gold())
        ...
        await interaction.response.send_message("削除したいメモを選んでください:", view=view, ephemeral=True)
...
async def my_clip(interaction: discord.Interaction, action: discord.app_commands.Choice[str], content: str = None):
    ...
    if act == "add":
        ...
        embed = discord.Embed(title="クリップ", color=discord.Color.magenta())
        embed.description = "\n".join([f"· {b}" for b in bks]) if bks else "保存されているクリップはありません。"
        await interaction.response.send_message(embed=embed, ephemeral=True)
    elif act == "clear":
        user_data["bookmarks"] = []
        save_data(all_data)
        await interaction.response.send_message("全てのクリップを消去しました。", ephemeral=True)
...
async def owner_status(interaction: discord.Interaction, text: str):
    ...
    try:
        if text.lower() == "reset":
            ...
            await interaction.response.send_message(f"ステータスを「{text} を視聴中」に変更しました。", ephemeral=True)
    except Exception as e:
...
async def my_scan_channels(interaction: discord.Interaction):
    ...
    full_rep = "\n".join(report)
    await interaction.followup.send(embed=discord.Embed(title="チャンネルスキャン結果", description=full_rep[:1950], color=discord.Color.red()), ephemeral=True)
...
async def my_audit_perms(interaction: discord.Interaction):
    ...
    for channel in interaction.guild.text_channels:
        ...
        if issues: report.append(f"· {channel.mention}  @everyone に「{', '.join(issues)}」権限があります")
...
async def server_status(interaction: discord.Interaction):
    ...
    embed.add_field(name="メッセージ転送", value=forward_status, inline=False)
    ...
    embed.add_field(name="サーバー認証", value=verify_status, inline=False)
    ...
    embed.add_field(name="配信 · お知らせ", value=announce_status, inline=False)
    ...
    embed.add_field(name="自動返信メンション", value=mention_status, inline=False)

    panel_roles_ids = cfg.get("panel_roles", [])
    valid_panel_roles = [g.get_role(rid).name for rid in panel_roles_ids if g.get_role(rid)]
    panel_status = f"登録済み ({len(valid_panel_roles)}個)\n`{', '.join(valid_panel_roles)}`" if valid_panel_roles else "未登録"
    embed.add_field(name="ロールパネル", value=panel_status, inline=False)

    allowed_users = cfg.get("allowed_users", [])
    allowed_status = ", ".join([f"<@{uid}>" for uid in allowed_users]) if allowed_users else "なし（管理者のみ使用可能）"
    embed.add_field(name="コマンド許可ユーザー", value=allowed_status, inline=False)
...
class GuildDetailSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="メンバー数",
            value=f"総計 **{total}人**  ·  人間 {humans}人  ·  Bot {bots}体",
            inline=False
        )
        ...
        embed.add_field(
            name="チャンネル数",
            value=f"テキスト {text_ch}  ·  ボイス {voice_ch}  ·  カテゴリー {category_ch}",
            inline=False
        )
        ...
        embed.add_field(name="Bot 設定状況", value="\n".join(settings), inline=False)
...
class BroadcastEmbedModal(discord.ui.Modal, title="お知らせ内容を入力"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        preview_embed.add_field(name="送信対象", value=f"{len(self.target_guilds)}サーバー", inline=False)
        preview_embed.add_field(name="タイトル", value=title_text, inline=False)
        preview_embed.add_field(name="本文", value=body_text[:500] + ("..." if len(body_text) > 500 else ""), inline=False)
...
class BroadcastColorSelect(discord.ui.Select):
    def __init__(self, target_guilds: list, channel_map: dict):
        ...
        super().__init__(placeholder="Embed のカラーを選択...", options=options)
