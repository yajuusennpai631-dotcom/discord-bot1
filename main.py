...
def get_guild_config(all_data, guild_id_str):
    if guild_id_str not in all_data:
        all_data[guild_id_str] = {
            ...
            "verify_role": None,
            "panel_roles": [],
            "mention_trigger_channel": None,  # 【新設】自動メンション監視ch
            "mention_target_role": None       # 【新設】自動メンション対象ロール
        }
...
async def on_ready():
    ...
    for guild_id_str, config in all_data.items():
        ...
        print(f"  > 自動メンション: {'有効' if config.get('mention_trigger_channel') else '未設定'}") # 追加
...
# サーバー追加・退出時に導入サーバー数をリアルタイム更新するイベント
@bot.event
...
async def on_message(message: discord.Message):
    ...
    if guild_id_str in all_data:
        ...
        if trigger_ch_id and target_role_id and message.channel.id == trigger_ch_id:
            ...
            if role:
                # 誰かが書き込んだ直後に、Botが指定されたロールにメンション付き通知を送信
                await message.channel.send(
                    f"{role.mention} 新しい書き込みがありました！", 
    ...
    if is_admin or is_owner:
        embed.add_field(
            ...
            value=(
                ...
                "`/server_mention_setup` / `reset` : 自動ロールメンションの設定と解除を行います" # 追加
            ),
            inline=False
        )
...
async def server_status(interaction: discord.Interaction):
    ...
    mention_status = f"有効\n・監視ch: {m_ch.mention if m_ch else '削除済'}\n・メンション対象ロール: {m_role.mention if m_role else '削除済'}" if (m_ch or m_role) else "未設定"
    embed.add_field(name="自動ロールメンション設定", value=mention_status, inline=False)
...
# ==================== 【【新設】自動ロールメンション設定コマンド (要・管理者権限)】 ====================

@bot.tree.command(name="server_mention_setup", description="指定チャンネルへの投稿時に、指定ロールを自動で即座にメンションする設定を行います")
@app_commands.default_permissions(administrator=True)
async def server_mention_setup(interaction: discord.Interaction, channel: discord.TextChannel, role: discord.Role):
    if not interaction.guild: return
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("このコマンドはサーバー管理者専用です。", ephemeral=True)
        return
    ...
    await interaction.response.send_message(
        ...
        f"※一般ユーザーを含むすべてのメッセージに対してBotが即座にメンション通知を追記します。", 
        ephemeral=True
    )


@bot.tree.command(name="server_mention_reset", description="自動ロールメンションの監視設定を解除します")
@app_commands.default_permissions(administrator=True)
async def server_mention_reset(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("自動ロールメンションの設定を解除しました。", ephemeral=True)
